...
PDF和图表的生成由执行方自行选择工具（Claude的pdf skill、KIMI自带的PDF能力、matplotlib+reportlab、weasyprint、puppeteer/playwright截图等均可），本skill不提供渲染脚本。视觉规范参考`references/report_style.md`。

执行方必须保证中文字体可用（Windows的微软雅黑、Linux的Noto Sans CJK或思源黑体等完整覆盖CJK的字体），否则PDF会出现乱码。其他格式（.xlsx、.md、单图）仅在用户明确要求时使用。
...
配色值和组件规范见`references/report_style.md`。图表和PDF报告的主题保持一致。
