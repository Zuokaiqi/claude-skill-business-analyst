...
不要每次都重写 PDF 生成代码。本技能提供了 [scripts/pdf_report.py](scripts/pdf_report.py)，封装了封面、标题、假设标签、结论高亮、建议动作框、表格、图表嵌入、风险标注的标准 API，已对齐 [references/report_style.md](references/report_style.md) 的视觉规范并自动处理跨平台中文字体。

依赖：`fpdf2 >= 2.7`、`pillow`。如果未安装：
...
不要再手动配置 rcParams 和颜色常量——本技能已经在 [scripts/chart_style.py](scripts/chart_style.py) 中固化好深色主题、跨平台中文字体探测和配色方案。同时 [scripts/pdf_report.py](scripts/pdf_report.py) 提供了 PDF 报告的高层 API。
...
from chart_style import (
    ...
    COLOR_SUCCESS, COLOR_DANGER, PALETTE,
)
from pdf_report import Report

# 模块加载时已自动 apply_style('dark')，正常情况下无需再调用
# 仅当需要单独导出浅色背景图时再用 apply_style('light')
```

`chart_style` 模块导入时会自动：
1. 跨平台寻找中文字体（Windows / macOS / Linux），注册到 matplotlib
2. 应用深色主题，与 PDF 报告样式严格一致
3. 设置 seaborn 调色板为本技能的 6 色方案

## 可视化

使用 seaborn + matplotlib，配色和主题由上面的 `apply_style('dark')` 自动接管。

图表原则：
- 标题说明"比较什么"或"展示什么趋势"
- 数据标签直接标注在图表上
- 配色优先使用粉色（#E91E8C）和蓝色（#4A9EF5）
- 如果涉及时间序列，X 轴必须是时间，不要用分类轴
- 对比图中加盈亏平衡线或基准线
